...
export function renderUsageLine(ctx: RenderContext): string | null {
  ...
  if (!usageData || !usageData.planName) return null;
...
function renderStandardUsage(ctx: RenderContext): string {
  const { usageData, config } = ctx;
  if (!usageData) return '';
  ...
  const fiveHourPart = renderQuotaSegment(
    '5h',
    usageData.fiveHour,
    usageData.fiveHourResetAt,
    barWidth,
    barStyle,
    colorScheme,
    effectiveTimeFormat,
    usageData.apiUnavailable ?? false,
  );

  const sevenDayPart = renderQuotaSegment(
    '7d',
    usageData.sevenDay,
    usageData.sevenDayResetAt,
    barWidth,
    barStyle,
    colorScheme,
    effectiveTimeFormat,
...
function renderDashboardUsage(ctx: RenderContext): string {
  const { usageData, config } = ctx;
  if (!usageData) return '';
  ...
  const fiveHourLine = renderDashboardQuotaLine(
    '5h',
    usageData.fiveHour,
    usageData.fiveHourResetAt,
    barWidth,
  ...
  const sevenDayLine = renderDashboardQuotaLine(
    '7d',
    usageData.sevenDay,
    usageData.sevenDayResetAt,
    barWidth,
...
function renderQuotaSegment(
  ...
  resetAt: Date | null,
  barWidth: number,
  barStyle: RenderContext['config']['barStyle'],
  colorScheme: RenderContext['config']['colorScheme'],
  timeFormat: RenderContext['config']['timeFormat'] | null,
  ...
  const colorFn = (s: string): string => `${colorCode}${s}${RESET}`;
...
function renderDashboardQuotaLine(
  label: string,
  percent: number | null,
  resetAt: Date | null,
  barWidth: number,
...
function formatResetTime(resetAt: Date, timeFormat: RenderContext['config']['timeFormat']): string {
  const now = new Date();
...
function formatAbsoluteTime(now: Date, resetAt: Date): string {
...
function formatRelative(now: Date, resetAt: Date): string {
