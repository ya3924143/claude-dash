...
export function renderBar(percent, width, style, colorFn) {
    switch (style) {
...
function renderBlockBar(percent, width, colorFn) {
...
function renderHalfBar(percent, width, colorFn) {
    const filled = Math.round((percent / 100) * width);
    const empty = width - filled;
    const filledPart = colorFn('▓'.repeat(filled));
    const emptyPart = dim('░'.repeat(empty));
...
function renderGradientBar(percent, width, colorFn) {
...
function renderAsciiBar(percent, width, colorFn) {
...
function renderCircleBar(percent) {
    if (percent < 25)
        return green('○');
    if (percent < 50)
        return brightBlue('◔');
...
export function renderContextCircle(percent, scheme) {
